...
for participantdir in `cat $1`; do
  ...
  Tractometry -i TOM_trackings_lesions/ -o Tractometry_lesions_NODDI_ND.csv -e endings_segmentations/ -s $dwidir/NODDI_ficvf_resampled.nii.gz

  if [ -f Tractometry_lesions_NODDI_ND.csv ]; then

    echo "Finished tractometry for $participant on $date" >> $projectdir/TractSeg_lesions.log

  else

    echo "Something went wrong for $participant - check it manually" >> $projectdir/TractSeg_lesions.log
