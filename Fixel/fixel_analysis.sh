...
mkdir -p ../template_random/fod_input
mkdir ../template_random/mask_input
...
foreach * : ln -sr IN/Brain/DWI/wmfod_norm.mif ../template_random/fod_input/PRE.mif
foreach * : ln -sr IN/Brain/DWI/dwi_mask_upsampled.mif ../template_random/mask_input/PRE.mif

#Find recursively an remove participants-THAT YOU DON'T WANT IN THE TEMPLATE for fixel template
...
for i in `cat fixel_random_sample_participants_to_delete.txt`; do
  find /home/pjakuszyk/seropositive_project/template_random -maxdepth 3  -name ${i}.mif -exec rm -rf {} \;
done
...
population_template template_random/fod_input -mask_dir template_random/mask_input template_random/wmfod_template.mif -voxel_size 1.25 -nthreads 16

#Register the FOD image from each subject to the FOD template:

foreach * : mrregister IN/Brain/DWI/wmfod_norm.mif -mask1 IN/Brain/DWI/dwi_mask_upsampled.mif ../template_random/wmfod_template.mif -nl_warp IN/Brain/DWI/subject2template_warp.mif IN/Brain/DWI/template2subject_warp.mif -force

#Compute the template mask (intersection of all subject masks in template space)
#To warp all masks into template space:

foreach * : mrtransform IN/Brain/DWI/dwi_mask_upsampled.mif -warp IN/Brain/DWI/subject2template_warp.mif -interp nearest -datatype bit IN/Brain/DWI/dwi_mask_in_template_space.mif -force

#Compute the template mask as the intersection of all warped masks:

mrmath */Brain/DWI/dwi_mask_in_template_space.mif min ../template_random/template_mask.mif -datatype bit -force

#Compute a white matter template analysis fixel mask

fod2fixel -mask ../template_random/template_mask.mif -fmls_peak_value 0.06 ../template_random/wmfod_template.mif ../template_random/fixel_mask
...
mrinfo -size ../template_random/fixel_mask/directions.mif
...
foreach * : mrtransform IN/Brain/DWI/wmfod_norm.mif -warp IN/Brain/DWI/subject2template_warp.mif -reorient_fod no IN/Brain/DWI/fod_in_template_space_NOT_REORIENTED.mif -force
...
foreach * : fod2fixel -mask ../template_random/template_mask.mif IN/Brain/DWI/fod_in_template_space_NOT_REORIENTED.mif IN/Brain/DWI/fixel_in_template_space_NOT_REORIENTED_random -afd fd.mif -force
...
foreach * : fixelreorient IN/Brain/DWI/fixel_in_template_space_NOT_REORIENTED_random IN/Brain/DWI/subject2template_warp.mif IN/Brain/DWI/fixel_in_template_space_random -force

#Assign subject fixels to template fixels

foreach * : fixelcorrespondence IN/Brain/DWI/fixel_in_template_space_random/fd.mif ../template_random/fixel_mask ../template_random/fd PRE.mif

#Compute the fibre cross-section (FC) metric

foreach * : warp2metric IN/Brain/DWI/subject2template_warp.mif -fc ../template_random/fixel_mask ../template_random/fc IN.mif
...
mkdir ../template_random/log_fc
cp ../template_random/fc/index.mif ../template_random/fc/directions.mif ../template_random/log_fc
foreach * : mrcalc ../template_random/fc/IN.mif -log ../template_random/log_fc/IN.mif

#Compute a combined measure of fibre density and cross-section (FDC)

mkdir ../template_random/fdc
cp ../template_random/fc/index.mif ../template_random/fdc
cp ../template_random/fc/directions.mif ../template_random/fdc
foreach * : mrcalc ../template_random/fd/IN.mif ../template_random/fc/IN.mif -mult ../template_random/fdc/IN.mif

#Perform whole-brain fibre tractography on the FOD template

cd ../template_random
tckgen -angle 22.5 -maxlen 250 -minlen 10 -power 1.0 wmfod_template.mif -seed_image template_mask.mif -mask template_mask.mif -select 20000000 -cutoff 0.06 tracks_20_million.tck -nthreads 32 -force
...
#HC - AQP4
fixelcfestats fd_smooth/ files.txt design_matrix.txt HC_AQP4_contrast.txt matrix/ stats_fd_HC_AQP4/ -nshuffles 500
#AQP4 - SM
...
#HC - AQP4
fixelcfestats log_fc_smooth/ files.txt design_matrix.txt HC_AQP4_contrast.txt matrix/ stats_log_fc_HC_AQP4/ -nshuffles 500
...
#SM - HC
fixelcfestats fdc_smooth/ files.txt design_matrix.txt SM_HC_contrast.txt matrix/ stats_fdc_SM_HC/ -nshuffles 500
